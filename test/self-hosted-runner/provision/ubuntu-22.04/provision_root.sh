...
# We use these images to create nri policy plugins so fetch it beforehand,
# see what is the needed image (and version) in cmd/{topology-aware|balloons}/Dockerfile
# Ignore errors for now, we can manually login to VM to pull the images.
docker image pull ${REGISTRY}golang:1.19-buster || true
docker image pull ${REGISTRY}gcr.io/distroless/static || true
...
# Make sure vagrant user can work with kvm
gpasswd -a vagrant kvm
