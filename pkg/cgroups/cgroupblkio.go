...
func readFromFileInDir(baseDir string, filenames []string) (string, error) {
	var errs error
	// If reading all the files fails, return list of read errs.
...
func writeToFileInDir(baseDir string, filenames []string, content string) error {
	var errs error
	// Returns list of errs from writes, list of single error due to all filenames missing or nil on success.
	for _, filename := range filenames {
		filepath := filepath.Join(baseDir, filename)
